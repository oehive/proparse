/*
 * MessageWindow.java
 * September 2003 by John Green
 * Based on org.eclipse.swt.examples.addressbook.
...
public class MessageWindow {
	...
	private static ResourceBundle resMessages = ResourceBundle.getBundle("resources/messagewindow");
	private static ResourceBundle resUser = ResourceBundle.getBundle("resources/usersettings");
	private Shell shell;
	...
	private static final String[] columnNames = {resMessages.getString("File_name"),
												 resMessages.getString("File_line"),
												 resMessages.getString("Message_message"),
												 resMessages.getString("Full_path")};
...
private boolean closeAddressBook() {
	if(isModified) {
		...
		box.setMessage(resMessages.getString("Close_save"));
...
private void newAddressBook() {	
	shell.setText(resMessages.getString("Title_bar") + resMessages.getString("New_title"));
	file = null;
	isModified = false;
}
...
private void openAddressBook() {	
	...
	fileDialog.setFilterNames(new String[] {resMessages.getString("Book_filter_name") + " (*.messages)", 
...
private void openAddressBook(String name) {
	...
	if (!file.exists()) {
		displayError(resMessages.getString("File")+file.getName()+" "+resMessages.getString("Does_not_exist")); 
		return;
	}
...
private boolean saveAs() {
	...
	saveDialog.setFilterExtensions(new String[] {"*.messages;",  "*.*"});
	saveDialog.setFilterNames(new String[] {"Address Books (*.messages)", "All Files "});
	...
	if(name.indexOf(".messages") != name.length() - 6) {
		name += ".messages";
	}
	...
	if(file.exists()) {
		...
		box.setText(resMessages.getString("Save_as_title"));
		box.setMessage(resMessages.getString("File") + file.getName()+" "+resMessages.getString("Query_overwrite"));
		if(box.open() != SWT.YES) {
			return false;
		}
	}
...
private void createFileMenu(Menu menuBar) {
	...
	item.setText(resMessages.getString("File_menu_title"));
	...
	subItem.setText(resMessages.getString("New_message"));
	...
	subItem.setText(resMessages.getString("Edit_message"));
	...
	subItem = new MenuItem(menu, SWT.NULL);
	subItem.setText(resMessages.getString("New_message_list"));
	...
	//File -> Open
	subItem = new MenuItem(menu, SWT.NULL);
	subItem.setText(resMessages.getString("Open_message_list"));
	...
	//File -> Save.
	subItem = new MenuItem(menu, SWT.NULL);
	subItem.setText(resMessages.getString("Save_message_list"));
	...
	//File -> Save As.
	subItem = new MenuItem(menu, SWT.NULL);
	subItem.setText(resMessages.getString("Save_list_as"));
	...
	//File -> Exit.
	subItem = new MenuItem(menu, SWT.NULL);
	subItem.setText(resMessages.getString("Exit"));
	subItem.addSelectionListener(new SelectionAdapter() {
		public void widgetSelected(SelectionEvent e) {
			shell.close();
		}
...
private MenuItem createEditMenu(Menu menuBar) {
	//Edit menu.
	MenuItem item = new MenuItem(menuBar, SWT.CASCADE);
	item.setText(resMessages.getString("Edit_menu_title"));
	Menu menu = new Menu(shell, SWT.DROP_DOWN);
	item.setMenu(menu);
	...
	subItem.setText(resMessages.getString("Edit"));
	subItem.setAccelerator(SWT.MOD1 + 'E');
	subItem.addSelectionListener(new SelectionAdapter() {
		public void widgetSelected(SelectionEvent e) {
			TableItem[] items = table.getSelection();
	...
	//Edit -> Copy
	subItem = new MenuItem(menu, SWT.NULL);
	subItem.setText(resMessages.getString("Copy"));
	...
	//Edit -> Paste
	subItem = new MenuItem(menu, SWT.NULL);
	subItem.setText(resMessages.getString("Paste"));
	...
	//Edit -> Delete
	subItem = new MenuItem(menu, SWT.NULL);
	subItem.setText(resMessages.getString("Delete"));
	subItem.addSelectionListener(new SelectionAdapter() {
		public void widgetSelected(SelectionEvent e) {
			TableItem[] items = table.getSelection();
			if (items.length == 0) return;
	...
	subItem.setText(resMessages.getString("Sort"));
...
private Menu createPopUpMenu() {
	...
	item.setText(resMessages.getString("Pop_up_new"));
	item.addSelectionListener(new SelectionAdapter() {
		public void widgetSelected(SelectionEvent e) {
			newEntry();
		}
	...
	item.setText(resMessages.getString("Pop_up_edit"));
	item.addSelectionListener(new SelectionAdapter() {
		public void widgetSelected(SelectionEvent e) {
			TableItem[] items = table.getSelection();
			if (items.length == 0) return;
	...
	//Copy
	item = new MenuItem(popUpMenu, SWT.CASCADE);
	item.setText(resMessages.getString("Pop_up_copy"));
	item.addSelectionListener(new SelectionAdapter() {
		public void widgetSelected(SelectionEvent e) {
			TableItem[] items = table.getSelection();
			if (items.length == 0) return;
	...
	//Paste
	item = new MenuItem(popUpMenu, SWT.CASCADE);
	item.setText(resMessages.getString("Pop_up_paste"));
	item.addSelectionListener(new SelectionAdapter() {
		public void widgetSelected(SelectionEvent e) {
			if (copyBuffer == null) return;
			TableItem item = new TableItem(table, SWT.NONE);
	...
	//Delete
	item = new MenuItem(popUpMenu, SWT.CASCADE);
	item.setText(resMessages.getString("Pop_up_delete"));
	item.addSelectionListener(new SelectionAdapter() {
		public void widgetSelected(SelectionEvent e) {
			TableItem[] items = table.getSelection();
			if (items.length == 0) return;
	...
	item.setText(resMessages.getString("Pop_up_find"));
	item.addSelectionListener(new SelectionAdapter() {
		public void widgetSelected(SelectionEvent e) {
			searchDialog.open();
		}
