...
Copyright (c) 2001-2009 Joanju Software.
...
constructorstate
	:	#(	CONSTRUCTOR (PUBLIC|PROTECTED|PRIVATE|STATIC)? TYPE_NAME function_params
...
definebrowsestate
	:	#(	DEFINE (def_shared)? (def_visib)? (STATIC)? BROWSE ID
...
definebufferstate
	:	#(	DEFINE (def_shared)? (def_visib)? (STATIC)? BUFFER ID
...
definebuttonstate
	:	#(	DEFINE (def_shared)? (def_visib)? (STATIC)? BUTTON ID
			(	AUTOGO
			|	AUTOENDKEY
			|	DEFAULT
			|	color_expr
...
definedatasetstate
	:	#(	DEFINE (def_shared)? (def_visib)? (STATIC)? DATASET ID
			(namespace_uri)? (namespace_prefix)? (xml_node_name)? (REFERENCEONLY)?
			FOR RECORD_NAME (COMMA RECORD_NAME)*
...
definedatasourcestate
	:	#(	DEFINE (def_shared)? (def_visib)? (STATIC)? DATASOURCE ID
			FOR (#(QUERY ID))?
...
defineframestate
	:	#(	DEFINE (def_shared)? (def_visib)? (STATIC)? FRAME ID
			(form_item)*
			(	#(HEADER (display_item)+ )
...
defineimagestate
	:	#(	DEFINE (def_shared)? (def_visib)? (STATIC)? IMAGE ID
			(	#(LIKE field (VALIDATE)?)
			|	imagephrase_opt 
			|	sizephrase
			|	color_expr
...
definemenustate
	:	#(	DEFINE (def_shared)? (def_visib)? (STATIC)? MENU ID
...
definequerystate
	:	#(	DEFINE (def_shared)? (def_visib)? (STATIC)? QUERY ID
...
definerectanglestate
	:	#(	DEFINE (def_shared)? (def_visib)? (STATIC)? RECTANGLE ID
			(	NOFILL
...
definestreamstate
	:	#(	DEFINE (def_shared)? (def_visib)? (STATIC)? STREAM ID state_end )
	;

definesubmenustate
	:	#(	DEFINE (def_shared)? (def_visib)? (STATIC)? SUBMENU ID
			(menu_opt)* (menu_list_item)* state_end
		)
	;
   
definetemptablestate
	:	#(	DEFINE (def_shared)? (def_visib)? (STATIC)? TEMPTABLE ID
			(UNDO|NOUNDO)?
			(namespace_uri)? (namespace_prefix)? (xml_node_name)?
			(REFERENCEONLY)?
			(def_table_like)?
...
defineworktablestate
	:	#(	DEFINE (def_shared)? (def_visib)? (STATIC)? WORKTABLE ID
			(NOUNDO)? (def_table_like)? (label_constant)? (def_table_field)* state_end
		)
	;

definevariablestate
	:	#(	DEFINE (def_shared)? (def_visib)? (STATIC)? VARIABLE ID
...
finallystate
	:	#(FINALLY blockorstate (EOF | #(END (FINALLY)?) state_end) )
	;
...
methodstate
	:	#(	METHOD (PRIVATE|PROTECTED|PUBLIC|STATIC|OVERRIDE|FINAL)*
